...
#define NOP  13                                   /*размер табл.операций    */
 ...
 struct ASSKARTA                                  /*структ.карты АССЕМБЛЕРА */
  {
   ...
   unsigned  char OPERAND  [13];                  /*поле операнда           */
...
*/
  ...
  union                                           /*определить об'единение  */
   {
    unsigned char BUF_OP_RR [2];                  /*определить буфер         */
  ...
  if ( PRNMET == 'Y' )                            /*если псевдооп.DC помеч.,*/
   {                                              /*то:                     */
	...
	T_SYM[ITSYM].ZNSYM = CHADR;                   /*   запомн. в табл.симв. */
      ...
      PRNMET = 'N';                               /*  занулить PRNMET зн.'N'*/
      CHADR = CHADR + 4;                          /* увелич.CHADR на 4      */
     }
    else
...
int SRX()                                         /*подпр.обр.опер.RX-форм. */
 {
  ...
  if ( !isdigit ( (int) *METKA1 ) )                /*если лексема начинается */
   {                                              /*с буквы, то:            */
    for ( J=0; J<=ITSYM; J++ )                    /* все метки исх.текста в */
     {                                            /* табл. T_SYM сравниваем */
						  /* со знач.перем. *METKA  */
 ...
 SRX1:
  ...
  if ( !isdigit ( (int) *METKA2 ) )                /*если лексема начинается */
   {                                              /*с буквы, то:            */
    for ( J=0; J<=ITSYM; J++ )                    /* все метки исх.текста в */
     {                                            /* табл. T_SYM сравниваем */
						  /* со знач.перем. *МЕТКА  */
...
int SSS()                                         /*подпр.обр.опер.SS-форм. */
{
  ...
  printf("%d\n", sscanf ((char*)TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND, "%8[^+]+%d(%d),%8s", op1, &offset, &length, op2));
  if (4 == sscanf ((char*)TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND, "%8[^+]+%d(%d),%8s", op1, &offset, &length, op2)) {
	  ...
	  if (4 == sscanf ((char*)TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND, "%8[^(](%d),%8[^(](%d)", op1, &length, op2, &length)) {
		printf ("%s %s %d\n", op1, op2, length);
		int ide1, ide2;
		if ((ide1 = get_symbol_index(op1)) < 0 || (ide2 = get_symbol_index(op2)) < 0) {
		    return 2;
...
*/
  ...
  if ( strcmp ( &NFIL [ strlen ( NFIL )-3 ], "ass" ) )
   ...
   {
    printf ( "%s\n", "Er4 Неверный тип файла с исходным текстом" );
    return -1;
   }
...

   {
    if ( (fp = fopen ( NFIL , "rb" )) == NULL )   /*при неудачн.открыт.ф-ла */
						  /* сообщение об ошибке    */
     {
      printf ( "%s %s\n", NFIL, "Er3 Не найден файл с исходным текстом" );
      return -1;
     }
	...
	if ( !fread ( ASSTEXT [I1], 80, 1, fp ) )
	 {
	  if ( feof ( fp ) )
	   ...
	   {
	    printf ( "%s\n", "Er1 Ошибка при чтении фыйла с исх.текстом" );
	    return -1;
	   }
	 }
       }

      printf ( "%s\n", "Er2 Переполнение буфера чтения исх.текста" );
      return -1;
     }
...
CONT3:
 ...
 T_MOP[2].BXPROG = SRR;                           /*команд АССЕМБЛЕРА при   */
 ...
 /**/
 for ( I1=0; I1<=ITSYM; I1++ )
 {
	 printf ("  %s = %d [%d]\n", T_SYM[I1].IMSYM, T_SYM[I1].ZNSYM, I1 );
 }
...
CONT5:
  if ( ITCARD == (RAB = SOBJFILE ()) )            /*если в об'ектный файл   */
   ...
   printf                                         /* сообшение об успешном  */
    (                                             /* завершении             */
     "%s\n",
     "Finish успешое завершение трансляции"
    );
  else                                            /*иначе:                  */
   {
    if ( RAB == -7 )
...
ERR1:
  printf ("%s\n","1 ошибка формата данных");        /*выдать диагностич.сообщ.*/
  goto CONT6;

ERR2:
  printf ("%s\n","2 Unknow identification");  /*выдать диагностич.сообщ.*/
  goto CONT6;

ERR3:
  printf ("%s\n","3 Error code operation");         /*выдать диагностич.сообщ.*/
  goto CONT6;

ERR4:
  printf ("%s\n","4 ошибка второго операнда");      /*выдать диагностич.сообщ.*/
  goto CONT6;

ERR5:
  printf ("%s\n","5 ошибка базирования");           /*выдать диагностич.сообщ.*/
  goto CONT6;

ERR6:
  printf ("%s\n","6 недопустимый номер регистра");  /*выдать диагностич.сообщ.*/
  goto CONT6;

ERR7:
  printf ("%s\n","7 ошибка открытия об'ектн.файла");/*выдать диагностич.сообщ.*/
  goto CONT6;


CONT6:
  printf ("%s%d\n","Error in the card N ",I1+1);     /*выдать диагностич.сообщ.*/
